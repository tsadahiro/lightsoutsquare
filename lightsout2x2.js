Vue.component('cell',{
    ...
    template: '<gv-on:click="push()" @mouseover="hover()"><rect width="100" height="100" v-bind:x="100*x" :y="100*y" stroke="black" :fill="pallet[v]" fill-opacity=0.5 ></rect><text v-bind:x="100*x" :y="100*y" stroke="black">a</text></g>',
    methods:{
	push: function(){
...
Vue.component('lightsout', {
    ...
    template: '<div><p>{{ msg }}</p><svg width="200" height="200"> <cell v-for="(s,id) in status" v-on:toggle="pushed($event)" v-bind:key=id :idx=s.idx v-bind:x=s.x v-bind:y=s.y v-bind:v=s.v></cell></svg></div>',
    methods:{
	pushed: function(key){
